...
describe('/api/users', () => {
  ...
  describe('GET /', () => {
    ...
    beforeEach(async () => {
      ...
      await User.bulkCreate([
        { name: 'mentor' , email: 'mentor@example.com', password_digest: 123456 },
    ...
    it('should return all users (stat code 200)', async () => {
      ...
      expect(res.body.some(u => u.name === 'mentor')).toBeTruthy();
      expect(res.body.some(u => u.email === 'mentor@example.com')).toBeTruthy();
  ...
  describe('POST /', () => {
    ...
    it('should return 400 if user is invalid', async () => {
      user_object = { email: 'mentor@example.com', password: '123' };
    ...
    it('should save user if user is valid', async () => {
      const res = await response(user_object);
      const user = await User.findOne({ where: { name: 'mentor' } });

      expect(res.status).toBe(200);
      expect(user).toHaveProperty('id');
      expect(user).toHaveProperty('name', 'mentor');
      expect(user).toHaveProperty('email', 'mentor@example.com');
